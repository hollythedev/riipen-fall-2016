 ...
 */
  ...
  */
 function riipen_entry_footer() {
 	// Hide category and tag text for pages.
 	if ( 'post' === get_post_type() ) {
 		/* translators: used between list items, there is a space after the comma */
 		$categories_list = get_the_category_list( esc_html( ', ' ) );
 		if ( $categories_list && riipen_categorized_blog() ) {
  ...
  if ( 'educator' === get_post_type() ) {
    /* translators: used between list items, there is a space after the comma */
    $categories_list = get_the_term_list( '', "insistution", esc_html( ', ' ) );
    if ( $categories_list && riipen_categorized_blog() ) {
      printf( '<span class="cat-links">' . esc_html( 'Posted in &rarr; %1$s' ) . '</span>', $categories_list ); // WPCS: XSS OK.
    }
    ...
    $tags_list = get_the_tag_list( '', esc_html( ', ' ) );
    if ( $tags_list ) {
      printf( '<span class="tags-links">' . esc_html( 'Teaches: %1$s' ) . '</span>', $tags_list ); // WPCS: XSS OK.
    }
  }
 }
  ...
  * @return bool
  */
 function riipen_categorized_blog() {
 	if ( false === ( $all_the_cool_cats = get_transient( 'riipen_categories' ) ) ) {
 		...
 		set_transient( 'riipen_categories', $all_the_cool_cats );
 	}

 	if ( $all_the_cool_cats > 1 ) {
 		// This blog has more than 1 category so riipen_categorized_blog should return true.
 		return true;
 	} else {
 		// This blog has only 1 category so riipen_categorized_blog should return false.
 		return false;
 	}
 }

 /**
  * Flush out the transients used in riipen_categorized_blog.
  */
 function riipen_category_transient_flusher() {
 	if ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) {
 		return;
 	}
 	// Like, beat it. Dig?
 	delete_transient( 'riipen_categories' );
 }
 add_action( 'edit_category', 'riipen_category_transient_flusher' );
 add_action( 'save_post',     'riipen_category_transient_flusher' );
  ...
  */
 function riipen_comment_list( $comment, $args, $depth ) {
  ...
  */
 function riipen_numbered_pagination() {
 	global $wp_query;
 	$big = 999999999;
